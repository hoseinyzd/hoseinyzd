<h1 align="center">Hey there, I'm Hossein 👋</h1>
<h3 align="center">🧠 Network Engineer | Python Developer | Web Designer | DevOps | Linux Specialist 🐧</h3>

---

### 🚀 About Me
- 🛰️ MSc in **Computer Networks**, focused on secure and scalable infrastructure design  
- 🐍 **Python Developer** — automation, data pipelines, API integrations  
- 🌐 Passionate **Web Designer**
- ⚙️ Skilled in **DevOps**, CI/CD pipelines, containerization, and server‑side deployments  
- 🧩 Deep Linux expertise — from kernel‑level customization to production‑grade networking setups  
- 💡 Advocate of open‑source tools, clean configurations and performance‑centric workflows

---

### 🧰 My Tech Stack
<p>
  <img src="https://img.shields.io/badge/Linux-Ubuntu%20%7C%20Kubuntu-blue?logo=linux" />
  <img src="https://img.shields.io/badge/Python-3.x-yellow?logo=python" />
  <img src="https://img.shields.io/badge/DevOps-Docker%20%7C%20CI%2FCD-green?logo=docker" />
  <img src="https://img.shields.io/badge/Web-HTML5%20%7C%20CSS3%20%7C%20JS-orange?logo=html5" />
  <img src="https://img.shields.io/badge/Framework-FastAPI%20%7C%20Flask-009688?logo=fastapi" />
  <img src="https://img.shields.io/badge/Automation-Bash%20%7C%20Zsh%20%7C%20Ansible-grey?logo=gnu-bash" />
</p>

---

### 🔗 Connect With Me
- 💼 GitHub: [github.com/hoseinyzd](https://github.com/hoseinyzd)  
- 🌍 Website: [ITcorv.ir](https://itcorv.ir)  
- 💬 Telegram: [@hoseinyzd](https://t.me/hoseinyzd)  
- ✉️ Email: hoseinyzdp@gmail.com  

---

<sub>⚡ Always exploring network automation, infrastructure monitoring, and DevOps optimization.</sub>
